...
module mips_core(/*AUTOARG*/
   ...
   // Internal signals
   wire [31:0]   pc, controlPC, nextpc,nextPC, nextnextpc;
   ...
   wire          in_dcd_valid,in_ex_valid; 
   wire		   	 brnch;
   wire [1:0]    prediction;

   wire [1:0]    Gpredict, Lpredict, Cpredict;
   wire [11:0]   gpht_addr; 
   wire [9:0]    lpht_addr [15:0];
   wire [3:0]    localBR;
   ...
   wire [1:0]    ex_prediction;

   // Fetch/*{{{*/
   
   // PC Management
   register #(32, text_start) PCReg(pc,controlPC, clk, ~stall, 1'b0, rst_b);
   register #(32, text_start+4) PCReg2(nextpc, nextPC, clk, ~stall,1'b0, rst_b);
...
/*}}}*/
   ...
   register #(1, 0) FT_ID_Validbit(dcd_valid,in_dcd_valid,clk,~stall,1'b0,rst_b);
   register #(32, 32'h50000000)FT_ID_Reg0(dcd_inst,inst,clk,~stall,~in_dcd_valid, rst_b); 
   register #(32, 0) FT_ID_Reg1(dcd_pc,pc,clk,~stall,~in_dcd_valid, rst_b);
   register #(32, 0) FT_ID_Reg2(dcd_nextpc,nextpc,clk, ~stall,~in_dcd_valid, rst_b); 
   /*}}}*/
   ...
   assign fwd_rs_valid = 
                {(ex_ctrl_we & ex_valid & (dcd_rs == ex_rd_num) & ((ex_fwd_src<1)|| (ex_fwd_src == 4))), 
                 (mem_ctrl_we & mem_valid & (dcd_rs == mem_rd_num) & ((mem_fwd_src<2)|| (mem_fwd_src == 4))
                 & ~(ex_ctrl_we & ex_valid & (dcd_rs == ex_rd_num))),
                 (wb_ctrl_we & wb_valid & (dcd_rs == wb_rd_num)
                 & ~(ex_ctrl_we & ex_valid & (dcd_rs == ex_rd_num))
                 & ~(mem_ctrl_we & mem_valid & (dcd_rs == mem_rd_num)))};
   assign fwd_rt_valid = 
                {(ex_ctrl_we & ex_valid & (dcd_rt == ex_rd_num) & ((ex_fwd_src<1)|| (ex_fwd_src == 4))), 
                 (mem_ctrl_we & mem_valid & (dcd_rt == mem_rd_num) & ((mem_fwd_src<2)|| (mem_fwd_src == 4))
 ...
 /*}}}*/
  ...
  //Depencency Detection/*{{{*/
   stall_unit dependU(.stall   (stall), 
    ...
    // Control Flow/*{{{*/
    SIPO_reg #(12, 12'hfff) GHR(gpht_addr, brnch, (ex_valid&ex_isBranch), clk, rst_b);
    PHT_memory GPHT(Gpredict, gpht_addr, (ex_valid&ex_isBranch), brnch, clk, rst_b); 
   
    branch_tracker BT(localBR, (isBranch & dcd_valid), dcd_pc, clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR1(lpht_addr[0], brnch,(ex_valid&ex_isBranch&(localBR==0)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR2(lpht_addr[1], brnch,(ex_valid&ex_isBranch&(localBR==1)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR3(lpht_addr[2], brnch,(ex_valid&ex_isBranch&(localBR==2)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR4(lpht_addr[3], brnch,(ex_valid&ex_isBranch&(localBR==3)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR5(lpht_addr[4], brnch,(ex_valid&ex_isBranch&(localBR==4)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR6(lpht_addr[5], brnch,(ex_valid&ex_isBranch&(localBR==5)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR7(lpht_addr[6], brnch,(ex_valid&ex_isBranch&(localBR==6)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR8(lpht_addr[7], brnch,(ex_valid&ex_isBranch&(localBR==7)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR9(lpht_addr[8], brnch,(ex_valid&ex_isBranch&(localBR==8)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR10(lpht_addr[9], brnch,(ex_valid&ex_isBranch&(localBR==9)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR11(lpht_addr[10], brnch,(ex_valid&ex_isBranch&(localBR==10)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR12(lpht_addr[11], brnch,(ex_valid&ex_isBranch&(localBR==11)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR13(lpht_addr[12], brnch,(ex_valid&ex_isBranch&(localBR==12)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR14(lpht_addr[13], brnch,(ex_valid&ex_isBranch&(localBR==13)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR15(lpht_addr[14], brnch,(ex_valid&ex_isBranch&(localBR==14)), clk, rst_b);
    SIPO_reg #(10, 10'h3ff) LHR16(lpht_addr[15], brnch,(ex_valid&ex_isBranch&(localBR==15)), clk, rst_b);
    L_PHT_memory LPHT(Lpredict, lpht_addr[localBR], (ex_valid&ex_isBranch), brnch, clk, rst_b); 
    
    PHT_memory CPHT(Cpredict, gpht_addr, (Gpredict != Cpredict),
                                         (brnch==Gpredict), clk, rst_b); 
    
    assign prediction = (Cpredict > 1) ? Gpredict : Lpredict;

    control_flow CFU(.nextPC         (nextPC),
                     .pc             (controlPC),
                     .nextpc         (nextpc),
                     .dcd_valid      (in_dcd_valid), 
                     .ex_valid       (in_ex_valid), 
                     .prediction     (prediction), 
                     .ex_prediction  (ex_prediction), 
                     .dcd_pc         (dcd_pc),
                     .dcd_nextpc     (dcd_nextpc),
                     .ex_pc          (ex_pc),
                     .ex_nextpc      (ex_nextpc),
                     .alu__out       (alu__out),
                     .dcd_target     (dcd_target), 
                     .ex_target      (ex_target),
                     .dcd_se_offset  (dcd_se_offset), 
                     .ex_se_offset   (ex_se_offset),
                     .isBranch       (isBranch),
   ...
   register #(1, 0) ID_EX_Validbit(ex_valid,in_ex_valid,clk,~internal_halt,stall, rst_b);
   ...
   register #(3, 3'hx) ID_EX_Reg22(ex_fwd_src, fwd_src,clk, ~internal_halt,(stall|~dcd_valid), rst_b); 
   ...
   register #(2, 2'b0) ID_EX_Reg32(ex_prediction, prediction, clk, ~internal_halt, (stall|~dcd_valid), rst_b); 
/*}}}*/
...
/*}}}*/
   ...
   register #(3, 3'hx) MEM_WB_Reg16(wb_fwd_src,mem_fwd_src,clk, ~internal_halt, 1'b0, rst_b);
...
module SIPO_reg (out, in, enable, clk, rst_b); 
...
module PHT_memory (entry_val, addr, we_h, newBranch,clk, rst_b); 
    
    output [1:0] entry_val;
    wire    [1:0] entry_val;
    input  [11:0] addr;
    input  we_h, newBranch, clk, rst_b;
    
    reg [1:0] memory[4095:0];
    integer i;
    ...
    always @ (posedge clk, negedge rst_b) begin
        if (~rst_b) begin
            ...
            if (((memory[addr] == 3) & newBranch)|| 
                ((memory[addr] == 0) & ~newBranch))
    ...
    assign entry_val = memory[addr];
endmodule 

module L_PHT_memory (entry_val, addr, we_h, newBranch,clk, rst_b); 
    ...
    reg [1:0] memory[1095:0];
    integer i;
   
    always @ (posedge clk, negedge rst_b) begin
        if (~rst_b) begin
            for (i=0; i < 1096; i = i +1) 
                memory[i] <= 2'b0;
        end 
        else if (we_h) begin
            if (((memory[addr] == 3) & newBranch)|| 
...
module branch_tracker (lregNum, isBranch, pc, clk, rst_b);
    ...
    reg [31:0]      brlist [15:0];
    reg [3:0]       count;
    integer         i;

    always @ (isBranch) begin
        if (brlist[0] == pc) lregNum = 0;
        else if(brlist[1] == pc) lregNum  = 1; 
        else if(brlist[2] == pc) lregNum  = 2; 
        else if(brlist[3] == pc) lregNum  = 3; 
        else if(brlist[4] == pc) lregNum  = 4; 
        else if(brlist[5] == pc) lregNum  = 5; 
        else if(brlist[6] == pc) lregNum  = 6; 
        else if(brlist[7] == pc) lregNum  = 7; 
        else if(brlist[8] == pc) lregNum  = 8; 
        else if(brlist[9] == pc) lregNum  = 9;
        else if(brlist[10] == pc) lregNum = 10;  
        else if(brlist[11] == pc) lregNum = 11;  
        else if(brlist[12] == pc) lregNum = 12;  
        else if(brlist[13] == pc) lregNum = 13;  
        else if(brlist[14] == pc) lregNum = 14; 
        else if(brlist[15] == pc) lregNum = 15;  
        else lregNum = count;
    end 
    ...
    always @ (posedge clk, negedge rst_b) begin
        if (~rst_b) begin
            count <=0;
            for(i=0; i<16; i = i+1)
                brlist[i] <= 32'hxxxx_xxxx;
        end
        else if (isBranch) begin
           brlist[count] <= (lregNum == count) ? pc : brlist[count];
           count <= (lregNum == count) ? count + 1 : count; 
        end
    end 
endmodule
...
module fowardingUnit (fwd_rs_data, fwd_rt_data,/*{{{*/
                         ...
                         ex_nextpc, mem_nextpc, wb_nextpc,
    ...
    input  [2:0]         ex_fwd_src,mem_fwd_src,wb_fwd_src;
    input  [31:0]        rs_data, rt_data;
    input  [31:0]        ex_nextpc, mem_nextpc, wb_nextpc;
    ...
    always @ * begin
        if ((valid_fwd_rs==3'b100)||(valid_fwd_rs==3'b101)||
            ...
            else if(ex_fwd_src == 4)
                fwd_rs_data = ex_nextpc;
            else
                fwd_rs_data = rs_data;
        end
            ...
            else if(mem_fwd_src == 4)
                fwd_rs_data = mem_nextpc;
            else
                fwd_rs_data = rs_data;
        end
            ...
            else if(wb_fwd_src == 4)
                fwd_rs_data = wb_nextpc;
            else
                fwd_rs_data = rs_data;
        end
        else 
...
module stall_unit(stall, /*{{{*/
    ...
    always @ (posedge clk, negedge rst_b) begin
        ...
        if(wroteBack & willWrite & (dest==wb_dest)) begin
            count[dest] <= count[dest];
        end
    end
endmodule/*}}}*/

module control_flow (nextPC,nextpc,pc, //{{{
                     dcd_valid,ex_valid, 
                     prediction, ex_prediction,
                     dcd_pc,dcd_nextpc, 
                     ex_pc, ex_nextpc,
                     alu__out, dcd_target, ex_target,
                     dcd_se_offset, ex_se_offset,
                     isBranch,ex_isBranch,
                     jump, ex_jump,
                     isALU,ex_isALU, isJal,ex_isJal,
                     realBR,stall);
    
    output [31:0] nextPC, pc;
    reg    [31:0] nextPC, pc;  
    output        dcd_valid,ex_valid;
    reg           dcd_valid,ex_valid;

    input  [1:0]  prediction, ex_prediction;
    input  [31:0] dcd_pc, dcd_nextpc, ex_pc, ex_nextpc;
    input  [31:0] nextpc;
    input  [31:0] alu__out;
    input  [25:0] dcd_target, ex_target;
    input  [31:0] dcd_se_offset, ex_se_offset;
    input         isBranch,ex_isBranch, jump, ex_jump;
    input         isALU,ex_isALU, isJal, ex_isJal;
    input         realBR;
    input         stall;
    
    wire takeBR, prev_takeBR;

    assign takeBR = (prediction>2'b01);
    assign prev_takeBR = (ex_prediction>2'b01);
    ...
    always @ * begin
            if(stall) begin
                ...
                case ({isALU,isJal})
                2'b01: begin
                    nextPC = {dcd_nextpc[31:28], dcd_target, 2'b0}+4;
                    pc = {dcd_nextpc[31:28], dcd_target, 2'b0};
                end
                2'b11: begin
                    nextPC = dcd_nextpc+dcd_se_offset+4;
                    pc = dcd_nextpc+dcd_se_offset; 
                end
                default: begin
                    nextPC = dcd_nextpc+4;
                    pc = dcd_nextpc; 
            ...
            if (ex_jump && ex_isALU && ~ex_isJal)begin
                ex_valid = 1'b0;
                dcd_valid = 1'b0;
                nextPC = alu__out+4;
                pc = alu__out;
            end
            else if (ex_isBranch) begin
                if(prev_takeBR != realBR) begin
                    ex_valid = 1'b0;
                    dcd_valid = 1'b0;
                    case ({isALU||realBR,isJal||realBR})
                    2'b01: begin
                        nextPC = {ex_nextpc[31:28], ex_target, 2'b0}+4;
                        pc = {ex_nextpc[31:28], ex_target, 2'b0};
                    end
                    2'b11: begin
                        nextPC = ex_nextpc+ex_se_offset+4;
                        pc = ex_nextpc+ex_se_offset; 
                    end
                    default: begin
                        nextPC = ex_nextpc+4;
                        pc = ex_nextpc; 
