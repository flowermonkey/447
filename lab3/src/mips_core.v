...
module mips_core(/*AUTOARG*/
   ...
   wire [1:0]    ex_regDest,mem_regDest,wb_regDest;
   ...
   register #(2, 0) ID_EX_Reg2(ex_regDest,regDest,clk, ~internal_halt,stall, rst_b);
   ...
   register #(32, 0) ID_EX_Reg13(ex_rs_data,rs_data,clk, ~internal_halt, stall, rst_b);
   register #(32, 0) ID_EX_Reg14(ex_rt_data,rt_data,clk, ~internal_halt, stall, rst_b);
   ...
   register #(5, 0) ID_EX_Reg18(ex_rd,dcd_rd,clk, ~internal_halt, stall, rst_b); 
   ...
   syscall_unit SU(.syscall_halt(syscall_halt), .pc(pc), .clk(clk), 
                        .Sys(ex_ctrl_Sys), .r_v0(rs_data), .rst_b(rst_b));
   ...
   register #(5, 0) EX_MEM_Reg6(mem_rd,ex_rd,clk, ~internal_halt, 1'b0, rst_b);
   register #(5, 0) EX_MEM_Reg7(mem_rt,ex_rt,clk, ~internal_halt, 1'b0, rst_b);
   ...
   register #(2, 0) EX_MEM_Reg10(mem_regDest,ex_regDest,clk, ~internal_halt, 1'b0, rst_b);
   ...
   register #(5, 0) MEM_WB_Reg0(wb_rd,mem_rd,clk, ~internal_halt, 1'b0, rst_b);
   register #(5, 0) MEM_WB_Reg1(wb_rt,mem_rt,clk, ~internal_halt, 1'b0, rst_b);
   ...
   register #(2, 0) MEM_WB_Reg7(wb_regDest,mem_regDest,clk, ~internal_halt, 1'b0, rst_b);
   ...
   mux2_1 #(5) syscallMux(rs_num,5'd2,dcd_rs,ex_ctrl_Sys);
   mux4_1 #(5)  writeReg(rd_num,wb_rd,wb_rt,5'd31,5'd31,wb_regDest);
   mux4_1 #(32) writeData(rd_data,wb_alu__out, wb_ld_mem_data,shiftVal,shiftVal,
					{wb_isShift,wb_memToReg});
   wire [4:0] sa;
...
module stall_unit(stall, dcd_rs,dcd_rt,dcd_rd,regDest,
                    pip_dest,willWrite,wroteBack,ctrl_Sys, clk, rst_b);
    output            stall;
    input      [4:0]  dcd_rs, dcd_rt, dcd_rd, pip_dest;
    input             willWrite, wroteBack, regDest,
                        ctrl_Sys, clk, rst_b;
